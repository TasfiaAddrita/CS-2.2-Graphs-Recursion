...
class TestReadGraphFromFile(unittest.TestCase):
    ...
    def test_get_all_vertices_n_away_flag(self):
        filename = 'test_files/graph_medium_undirected_2.txt'
        graph = read_graph_from_file(filename)

        vertices_1_away = graph.find_vertices_n_away('B', 1)
        self.assertEqual(sorted(vertices_1_away), ['A', 'C', 'D', 'Z'])

        vertices_2_away = graph.find_vertices_n_away('A', 2)
        self.assertEqual(sorted(vertices_2_away), ['D', 'E', 'Z'])

        vertices_3_away = graph.find_vertices_n_away('Z', 3)
        self.assertEqual(sorted(vertices_3_away), ['E', 'F'])
